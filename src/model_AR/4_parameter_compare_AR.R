## ---- PARAMETER COMPARE: MODEL AR (ALPHA AND RHO) ----
...
for (i in 1:length(sects)){
  ...
  file_name <- sprintf("../../out/EstData/param_est__compare_AR_%s.RData", sects[i])
  save(samples, file = file_name)
  print(sprintf("Samples saved at %s", file_name))
  file_names <- append(file_names, file_name)
}

save(sects, file_names, file = "../../out/EstData/est_compare_vars_AR.RData")
